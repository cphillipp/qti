...
define('INCEPTIO_THEME_VERSION', '1.1');
...
define('OPTION_CRSS_TYPE', 'inceptio_rss_type');
...
if (!function_exists('inceptio_setup')) {
    function inceptio_setup()
    {
        ...
        add_action('admin_print_scripts', 'inc_admin_scripts');
        add_action('admin_print_styles', 'inc_admin_styles');
...
if (!function_exists('inc_scripts_styles')) {
    function inc_scripts_styles()
    {
        ...
        wp_enqueue_style('color-style', $template_uri . '/css/colors/' . inc_get_site_color_scheme() . '.css', array('inceptio-style', 'inceptio-ie'), INCEPTIO_THEME_VERSION);
        ...
        if (IS_BROWSER_COMPATIBILITY_CHECK_ENABLED) {
            wp_enqueue_script('browser-check', $template_uri . '/js/ie.js', array('jquery'), INCEPTIO_THEME_VERSION, true);
        }
...
if (!function_exists('inc_admin_styles')) {
    function inc_admin_styles()
    {
        $current_screen = get_current_screen();
        if ($current_screen->id != 'media-upload') {
            $template_uri = get_template_directory_uri();
    ...
    function inc_portfolio_template_redirect()
    {
        ...
        if ($wp_query->query_vars["post_type"] == "portfolio") {
            if (have_posts()) {
                ...
                include(INCEPTIO_ROOT_PATH . '/template-portfolio-' . $template . '.php');
                die();
            } else {
