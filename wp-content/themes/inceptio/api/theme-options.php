...
class Inceptio_Theme_Options
{
    ...
    function register_theme_options_menu()
    {
        ...
        add_theme_page($theme_options_title, $theme_options_title, 'manage_options',
            'inceptio-theme-options', array($this, 'render'));
    }
    ...
    function render()
    {
        ...
        echo "<div class=\"widgets-full-width\">\n";
        foreach ($this->sections as $i => $section) {
...
class Inceptio_Section
{
    ...
    function render_title($is_selected)
    {
        ...
        echo '<a class="nav-tab' . $selected_tab_class . '" href="themes.php?page=inceptio-theme-options&tab=' . $this->id . '">' . $this->title . '</a>';
    }

    function render_content($as_hidden)
    {
        foreach ($this->categories as $i => $category) {
            $category->render($as_hidden);
        }
    }
}
...
class Inceptio_Category
{
    ...
    function render($as_hidden = false)
    {
        if ($as_hidden) {
            ...
            $section_state = $this->is_expanded ? '' : ' closed';

            echo '<div class="widgets-holder-wrap' . $section_state . '">';
            echo '<div class="sidebar-name section-title">';
            echo '<div class="sidebar-name-arrow"></div>';
...
class Inceptio_Option
{
    ...
    function __construct($element)
    {
        ...
        if (isset($element->default_value)) {
            if ($this->type == 'gradient') {
                ...
                $this->default_value = htmlspecialchars((string)$element->default_value);
            }
        }
    ...
    function print_hidden_field()
    {
        ...
        if (is_array($field_value)) {
            ...
            $field_value = esc_textarea(strip_tags($field_value));
