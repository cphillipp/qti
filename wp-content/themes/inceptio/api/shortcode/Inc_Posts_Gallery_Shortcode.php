...
class Inc_Posts_Gallery_Shortcode extends Abstract_Inc_Shortcode implements Inc_Shortcode_Designer
{
    ...
    function render($attr, $inner_content = null, $code = "")
    {
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                $query->the_post();
                $content .= $this->get_post_gallery_item($display_mode, $type, $columns, $tca);
                global $post;
                $post_terms = get_the_terms($post, $taxonomy);
                if ($post_terms) {
            ...
            if ($display_mode != 'thumbs' && $display_filters == 'true') {
                ...
                foreach ($posts_terms as $key => $val) {
                    $filter_content .= '<li><a href="#" data-filter=".' . $val . '">' . $key . '</a></li>';
                }
    ...
    private function get_post_gallery_item($display_mode, $type, $columns, $tca)
    {
        global $post;
        ...
        if ($display_mode == 'thumbs') {
            ...
            $content = '<li><a href="' . $post_url . '" title="' . $post_title . '"><img src="' . $img_src . '" alt=""></a></li>';
            return $content;
        } else {
            $li_class = "entry";
            if ($display_mode == 'gallery') {
                ...
                foreach ($assoc_media as $media) {
                    ...
                    if ($media['type'] == 'img') {
                        ...
                        if ($img_post) {
                            ...
                            $content .= "<a class=\"$a_class\" href=\"$post_url\" title = \"$img_title\"><span class=\"overlay\"></span><img src=\"$thumb_src\" alt=\"\"></a>";
                        }
                    } elseif ($media['type'] == 'video') {
                        $video_meta = $media['meta'];
                        if ($video_meta) {
                            ...
                            $content .= "<a class=\"$a_class\" href=\"$post_url\" title=\"$video_title\"><span class=\"overlay\"></span><img src=\"$video_thumb\" alt=\"\"></a>";
                        }
                    }
                    break;
                }
                ...
                foreach ($assoc_media as $media) {
                    ...
                    if ($media['type'] == 'img') {
                        ...
                        if ($img_post) {
                            ...
                            $content .= "<a class=\"$a_class\" rel=\"$gallery_id\" href=\"$full_src\" title = \"$img_title\"><span class=\"overlay zoom\"></span><img src=\"$thumb_src\" alt=\"\"></a>";
                        }
                    } elseif ($media['type'] == 'video') {
                        $video_meta = $media['meta'];
                        if ($video_meta) {
                            ...
                            $content .= "<a class=\"$a_class\" href=\"$video_url\" title=\"$video_title\"><span class=\"overlay zoom\"></span><img src=\"$video_thumb\" alt=\"\"></a>";
                        }
                    }
                    $i++;
                }
    ...
    function get_visual_editor_form()
    {
        ...
        $content .= '<input id="sc-posts-ids" name="sc-posts-ids" class="required" type="text" data-attr-name="' . Inc_Posts_Gallery_Shortcode::$IDS_ATTR . '" data-attr-type="attr">';
        $content .= '</div>';
        $content .= '<div>';
