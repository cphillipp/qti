...
class Inc_Iconbox_Shortcode extends Abstract_Inc_Shortcode implements Inc_Shortcode_Designer
{
    ...
    function render($attr, $inner_content = null, $code = "")
    {
        ...
        $img_alt = $title;
        $id = empty($id) ? "id=\"".sanitize_title($title)."\"" : "id=\"$id\"";
        ...
        if ($icon_position == 'top') {
            ...
            $content = "<div $id class=\"iconbox icon-top\">";
            if (!empty($href)) {
            ...
            $img_src = $this->get_icon_src($icon, $icon_size);
            $content = "<div $id class=\"iconbox icon-left\">";
            if (!empty($href)) {
                $content .= "<a $href $target>";
            }
            $content .= "<h2 class=\"iconbox-title\"><span class=\"iconbox-icon\"><img src=\"$img_src\" alt=\"$img_alt\"></span>$title</h2>";
            $content .= "$inner_content";
            $content .= $hl_text;
            if (!empty($href)) {
                $content .= $learn_more;
