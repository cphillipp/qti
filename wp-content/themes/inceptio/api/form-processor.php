...
if (!function_exists('inc_process_contact_form')) {
    function inc_process_contact_form()
    {
        try {
            ...
            $params_ignored = array('submit', 'action', 'ua', 'recaptcha_challenge_field', 'recaptcha_response_field');
            $email = $_POST['email'];
            ...
            $email_body .= "<p>Hi,</p>";
            $email_body .= "<p>You have just received an email from the address <a href=\"$current_host\" target=\"_blank\">$current_host</a> through the contact form.</p>";
            $email_body .= "<p>Bellow you have the data in the contact form:</p>";
            $email_body .= "<div>";
